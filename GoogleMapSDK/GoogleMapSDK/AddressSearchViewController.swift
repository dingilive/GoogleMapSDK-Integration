...
class AddressSearchViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, UITextFieldDelegate, GMSMapViewDelegate {
...
//        tableView.isHidden = true
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        DispatchQueue.main.async {
            ...
            self.mapView.clear()
            let marker = GMSMarker(position: CLLocationCoordinate2D(latitude: self.Result[indexPath.row].location[0], longitude: self.Result[indexPath.row].location[1]))
            marker.snippet = self.Result[indexPath.row].name
...
protocol AddrSearchDelegate: class {
    func AddrSearchBackPressed()
