...
MGL_EXPORT
/**
 `DingiImageSource` is a content source that is used for a georeferenced raster
 ...
 use the `-[DingiStyle setImage:forName:]` method. To configure a point
 annotation’s image, use the `DingiAnnotationImage` class.
 
 The geographic location of the raster image content, supplied with 
 `DingiCoordinateQuad`, can be non-axis aligned.
 `DingiImageSource` supports raster content from `NSURL`, `NSImage` (macOS), or
 `UIImage` (iOS).
 An image source is added to an `DingiStyle` object along with one or more
 ...
 `DingiImageSource` object that you can use to initialize new style layers. You
 ...
 */
- (instancetype)initWithIdentifier:(NSString *)identifier coordinateQuad:(DingiCoordinateQuad)coordinateQuad URL:(NSURL *)url;
...
/**
 ...
 @return An initialized shape source.
 */
- (instancetype)initWithIdentifier:(NSString *)identifier coordinateQuad:(DingiCoordinateQuad)coordinateQuad image:(MGLImage *)image;
 ...
 */
@property (nonatomic) DingiCoordinateQuad coordinates;
@end
