...
/**
 An `DingiHeatmapStyleLayer` is a style layer that renders a <a
 ...
 features can come from vector tiles loaded by an `DingiVectorTileSource` object,
 or they can be `DingiPointAnnotation`, `DingiPointFeature`, `DingiPointCollection`,
 or `DingiPointCollectionFeature` instances in an `DingiShapeSource` or
 `DingiComputedShapeSource` object.
 
 Consider accompanying a heatmap style layer with an `DingiCircleStyleLayer` or
 `DingiSymbolStyleLayer` at high zoom levels. If you are unsure whether the point
 data in an `DingiShapeSource` is dense enough to warrant a heatmap, you can
 alternatively cluster the source using the `DingiShapeSourceOptionClustered`
 option and render the data using an `DingiCircleStyleLayer` or
 ...
 ```swift
 let layer = DingiHeatmapStyleLayer(identifier: "earthquake-heat", source: earthquakes)
...
*/
@property (nonatomic) DingiTransition heatmapIntensityTransition;
...
*/
@property (nonatomic) DingiTransition heatmapOpacityTransition;
...
*/
@property (nonatomic) DingiTransition heatmapRadiusTransition;
