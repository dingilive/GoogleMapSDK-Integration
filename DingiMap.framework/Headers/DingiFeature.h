...
/**
 ...
 handled in `-[DingiMapViewDelegate mapView:didSelectAnnotation:]`.
 */
...
/**
 ...
 You can configure an `DingiVectorStyleLayer` object to include or exclude a
 specific feature in an `DingiShapeSource` or `DingiVectorTileSource`. In the
 `DingiVectorStyleLayer.predicate` property, compare the special `$id` attribute
 to the feature’s identifier.

 In vector tiles loaded by `DingiVectorTileSource` objects, the identifier
 corresponds to the
 ...
 The identifier should be set before adding the feature to an `DingiShapeSource`
 ...
 `-[DingiMapView visibleFeaturesAtPoint:]` and related methods, doing so likewise
...
/**
 ...
 You can configure an `DingiVectorStyleLayer` object to include or exclude a
 specific feature in an `DingiShapeSource` or `DingiVectorTileSource`. In the
 `DingiVectorStyleLayer.predicate` property, compare a key of the attribute
 dictionary to the value you want to include. For example, if you want an
 `DingiLineStyleLayer` object to display only important features, you might assign
 a value above 50 to the important features’ `importance` attribute, then set
 `DingiVectorStyleLayer.predicate` to an `NSPredicate` with the format
 `importance > 50`.

 You can also configure many layout and paint attributes of an `DingiStyleLayer`
 ...
 `DingiShapeSource` using an `DingiCircleStyleLayer`, you can assign a `halfway`
 attribute to each of the source’s features, then set
 `DingiCircleStyleLayer.circleRadius` to an `DingiStyleValue` object with an
 interpolation mode of `MGLInterpolationModeIdentity` and an attribute name of
 `halfway`.
 
 The `DingiSymbolStyleLayer.textField` and `DingiSymbolStyleLayer.iconImageName`
 properties allow you to use attributes yet another way. For example, to label
 features in an `DingiShapeSource` object by their names, you can assign a `name`
 attribute to each of the source’s features, then set
 `DingiSymbolStyleLayer.textField` to an `DingiStyleValue` object containing the
 raw string value `{name}`.

 In vector tiles loaded by `DingiVectorTileSource` objects, the keys and values of
 ...
 When adding a feature to an `DingiShapeSource`, use the same Foundation types
 ...
 when the feature is added to an `DingiShapeSource`. This can be convenient when
 ...
 instances obtained from `-[DingiMapView visibleFeaturesAtPoint:]` and related
 methods, there will be no effect on the map. Setting this value can be useful
 when the feature instance is used to initialize an `DingiShapeSource` and that
...
/**
 ...
 of an instance of an `DingiFeature` subclass.
...
/**
 An `DingiPointFeature` object associates a point shape with an optional
...
/**
 An `DingiPolylineFeature` object associates a polyline shape with an optional
 identifier and attributes.
...
/**
 An `DingiPolygonFeature` object associates a polygon shape with an optional
 identifier and attributes.
 */
MGL_EXPORT
...
/**
 An `DingiPointCollectionFeature` object associates a point collection with an
...
@compatibility_alias DingiMultiPointFeature DingiPointCollectionFeature;

/**
 An `DingiMultiPolylineFeature` object associates a multipolyline shape with an
 optional identifier and attributes.
...
/**
 An `DingiMultiPolygonFeature` object associates a multipolygon shape with an
 optional identifier and attributes.
 */
MGL_EXPORT
...
/**
 An `DingiShapeCollectionFeature` object associates a shape collection with an
 optional identifier and attributes.
 
 `DingiShapeCollectionFeature` is most commonly used to add multiple shapes to a
 single `DingiShapeSource`. Configure the appearance of an `DingiSource`’s shape
 collection collectively using an `DingiSymbolStyleLayer` object, or use multiple
 instances of `DingiCircleStyleLayer`, `DingiFillStyleLayer`, and
 `DingiLineStyleLayer` to configure the appearance of each kind of shape inside
 the collection.
...
@property (nonatomic, copy, readonly) NSArray<DingiShape<DingiFeature> *> *shapes;

+ (instancetype)shapeCollectionWithShapes:(NSArray<DingiShape<DingiFeature> *> *)shapes;
