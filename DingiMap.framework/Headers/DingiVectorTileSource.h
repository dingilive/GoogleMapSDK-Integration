...
/**
 `DingiVectorTileSource` is a map content source that supplies tiled vector data
 ...
 A vector tile source is added to an `DingiStyle` object along with one or more
 `DingiVectorStyleLayer` objects. A vector style layer defines the appearance of
 any content supplied by the vector tile source.
 
 `DingiVectorTileSource` is optimized for data sets that are too large to fit
 ...
 <a href="http://geojson.org/">GeoJSON</a> data, use the `DingiShapeSource`
 class. For tiled data that changes dynamically, the `DingiComputedShapeSource`
 ...
 `DingiVectorTileSource` object that you can use to initialize new style layers.
 ...
 ```swift
 let source = DingiVectorTileSource(identifier: "pois", tileURLTemplates: ["https://example.com/vector-tiles/{z}/{x}/{y}.mvt"], options: [
     .minimumZoomLevel: 9,
     .maximumZoomLevel: 16,
     .attributionInfos: [
...
/**
 ...
 @param options A dictionary containing configuration options. See
    `DingiTileSourceOption` for available keys and values. Pass in `nil` to use
    the default values.
 @return An initialized tile source.
 */
- (instancetype)initWithIdentifier:(NSString *)identifier tileURLTemplates:(NSArray<NSString *> *)tileURLTemplates options:(nullable NSDictionary<DingiTileSourceOption, id> *)options NS_DESIGNATED_INITIALIZER;
...
/**
 ...
 source loads a feature if the source is added to an `DingiMapView`’s style; that
 ...
 */
- (NSArray<id <DingiFeature>> *)featuresInSourceLayersWithIdentifiers:(NSSet<NSString *> *)sourceLayerIdentifiers predicate:(nullable NSPredicate *)predicate NS_SWIFT_NAME(features(sourceLayerIdentifiers:predicate:));
