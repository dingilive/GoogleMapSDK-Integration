...
/**
 ...
 Values of this type are passed to the `DingiMapView`'s delegate in the following methods:
 ...
 so if you'll often find `DingiCameraChangeReasonGesturePinch` set alongside `DingiCameraChangeReasonGestureRotate`.
 ...
 ```
 static const DingiCameraChangeReason anyZoom = DingiCameraChangeReasonGesturePinch |
                                                DingiCameraChangeReasonGestureZoomIn |
                                                DingiCameraChangeReasonGestureZoomOut |
                                                DingiCameraChangeReasonGestureOneFingerZoom;

 static const DingiCameraChangeReason anyRotation = DingiCameraChangeReasonResetNorth | DingiCameraChangeReasonGestureRotate;
 ```
 */
typedef NS_OPTIONS(NSUInteger, DingiCameraChangeReason)
{
    /// :nodoc: The reason for the camera change has not be specified.
    DingiCameraChangeReasonNone = 0,

    /// :nodoc: Set when a public API that moves the camera is called. This may be set for some gestures,
    /// for example DingiCameraChangeReasonResetNorth.
    DingiCameraChangeReasonProgrammatic = 1 << 0,

    /// :nodoc: The user tapped the compass to reset the map orientation so North is up.
    DingiCameraChangeReasonResetNorth = 1 << 1,

    /// :nodoc: The user panned the map.
    DingiCameraChangeReasonGesturePan = 1 << 2,

    /// :nodoc: The user pinched to zoom in/out.
    DingiCameraChangeReasonGesturePinch = 1 << 3,

    // :nodoc: The user rotated the map.
    DingiCameraChangeReasonGestureRotate = 1 << 4,

    /// :nodoc: The user zoomed the map in (one finger double tap).
    DingiCameraChangeReasonGestureZoomIn = 1 << 5,

    /// :nodoc: The user zoomed the map out (two finger single tap).
    DingiCameraChangeReasonGestureZoomOut = 1 << 6,

    /// :nodoc: The user long pressed on the map for a quick zoom (single tap, then long press and drag up/down).
    DingiCameraChangeReasonGestureOneFingerZoom = 1 << 7,

    // :nodoc: The user panned with two fingers to tilt the map (two finger drag).
    DingiCameraChangeReasonGestureTilt = 1 << 8,

    // :nodoc: Cancelled
    DingiCameraChangeReasonTransitionCancelled = 1 << 16
