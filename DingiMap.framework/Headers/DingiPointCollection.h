...
/**
 An `DingiPointCollection` object represents a shape consisting of one or more
 ...
 `DingiVectorTileSource`’s point collections collectively using an
 `DingiCircleStyleLayer` or `DingiSymbolStyleLayer` object. To access a point
 collection’s attributes, use an `DingiPointCollectionFeature` object.

 You cannot add an `DingiPointCollection` object directly to a map view as an
 annotation. However, you can create individual `DingiPointAnnotation` objects
 from the `coordinates` array and add those annotation objects to the map view
 using the `-[DingiMapView addAnnotations:]` method.

 A point collection is known as a
 <a href="https://tools.ietf.org/html/rfc7946#section-3.1.3">MultiPoint</a>
 geometry in GeoJSON. Do not confuse `DingiPointCollection` with `DingiMultiPoint`,
 the abstract superclass of `DingiPolyline` and `DingiPolygon`.
 */
MGL_EXPORT
@interface DingiPointCollection : DingiShape <DingiOverlay>

/**
 Creates and returns a `DingiPointCollection` object from the specified set of
 coordinates.
