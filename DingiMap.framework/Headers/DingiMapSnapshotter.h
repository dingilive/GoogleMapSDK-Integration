 ...
 */
- (instancetype)initWithStyleURL:(nullable NSURL *)styleURL camera:(DingiMapCamera *)camera size:(CGSize)size;
 ...
 */
@property (nonatomic) DingiMapCamera *camera;
 ...
 */
@property (nonatomic) DingiCoordinateBounds coordinateBounds;
...
/**
 ...
 @param snapshot The `DingiMapSnapshot` that was generated or `nil` if an error
    occurred.
 @param error The error that occured or `nil` when successful.
 */
typedef void (^DingiMapSnapshotCompletionHandler)(DingiMapSnapshot* _Nullable snapshot, NSError* _Nullable error);

/**
 An `DingiMapSnapshotter` generates static raster images of the map. Each snapshot
 image depicts a portion of a map defined by an `DingiMapSnapshotOptions` object
 you provide. The snapshotter generates an `DingiMapSnapshot` object
 ...
 For an interactive map, use the `DingiMapView` class. Both `DingiMapSnapshotter`
 and `DingiMapView` are compatible with offline packs managed by the
 `DingiOfflineStorage` class.
 ...
 */
- (instancetype)initWithOptions:(DingiMapSnapshotOptions *)options;
 ...
 */
- (void)startWithCompletionHandler:(DingiMapSnapshotCompletionHandler)completionHandler;
...
/**
 ...
 @param completionHandler The block to handle the result in.
 */
- (void)startWithQueue:(dispatch_queue_t)queue completionHandler:(DingiMapSnapshotCompletionHandler)completionHandler;

/**
 Cancels the snapshot creation request, if any.
 
 Once you call this method, you cannot resume the snapshot. In order to obtain
 the snapshot, create a new `DingiMapSnapshotter` object.
 */
- (void)cancel;
 ...
 */
@property (nonatomic) DingiMapSnapshotOptions *options;
