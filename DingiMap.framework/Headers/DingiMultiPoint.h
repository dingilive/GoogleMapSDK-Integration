...
/**
 The `DingiMultiPoint` class is an abstract superclass used to define shapes
 composed of multiple vertices.

 Create instances of `DingiPolyline` or `DingiPolygon` in order to use 
 properties of `DingiMultiPoint`. Do not create instances of `DingiMultiPoint`
 ...
 Do not confuse `DingiMultiPoint` with `DingiPointCollection`, which represents a
...
/**
 ...
 immediately. If the shape is part of an `DingiShapeSource` object, you must
 explicitly set the `DingiShapeSource.shape` property in order for any style
