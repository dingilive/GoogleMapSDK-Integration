...
/**
 `DingiVectorStyleLayer` is an abstract superclass for style layers whose content
 is defined by an `DingiShapeSource` or `DingiVectorTileSource` object.

 Create instances of `DingiCircleStyleLayer`, `DingiFillStyleLayer`,
 `DingiFillExtrusionStyleLayer`, `DingiHeatmapStyleLayer`, `DingiLineStyleLayer`, and
 `DingiSymbolStyleLayer` in order to use `DingiVectorStyleLayer`'s properties and
 methods. Do not create instances of `DingiVectorStyleLayer` directly, and do not
...
/**
 ...
 ```swift
 let layer = DingiLineStyleLayer(identifier: "contour", source: terrain)
