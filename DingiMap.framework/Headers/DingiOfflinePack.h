 ...
 */
typedef NS_ENUM (NSInteger, DingiOfflinePackState) {
    /**
     ...
     the time the shared `DingiOfflineStorage` object sends the first
     `DingiOfflinePackProgressChangedNotification` about the pack. For inactive
     packs, you must explicitly request a progress update using the
     `-[DingiOfflinePack requestProgress]` method.

     An invalid pack always has a state of `DingiOfflinePackStateInvalid`, never
     `DingiOfflinePackStateUnknown`.
     */
    DingiOfflinePackStateUnknown = 0,
    /**
     The pack is incomplete and is not currently downloading.

     This is the initial state of a pack that is created using the
     `-[DingiOfflineStorage addPackForRegion:withContext:completionHandler:]`
     method, as well as after the `-[DingiOfflinePack suspend]` method is
     called.
     */
    DingiOfflinePackStateInactive = 1,
    /**
     The pack is incomplete and is currently downloading.

     This is the state of a pack after the `-[DingiOfflinePack resume]` method is
     called.
     */
    DingiOfflinePackStateActive = 2,
    /**
     The pack has downloaded to completion.
     */
    DingiOfflinePackStateComplete = 3,
    /**
     The pack has been removed using the
     `-[DingiOfflineStorage removePack:withCompletionHandler:]` method. Sending
     any message to the pack will raise an exception.
     */
    DingiOfflinePackStateInvalid = 4,
};
...
/**
 ...
 progress.
 */
typedef struct __attribute__((objc_boxable)) DingiOfflinePackProgress {
    /**
 ...
 */
@property (nonatomic, readonly) id <DingiOfflineRegion> region;
...
/**
 ...
 `DingiOfflinePackStateUnknown` by default. To request the pack’s status, use the
 ...
 `DingiOfflinePackProgressChangedNotification`s about this pack that come from the
 default notification center.
 */
@property (nonatomic, readonly) DingiOfflinePackState state;
...
/**
 ...
 default notification center.
 */
@property (nonatomic, readonly) DingiOfflinePackProgress progress;
...
/**
 ...
 set to `DingiOfflinePackStateUnknown`, observe KVO change notifications on this
 pack’s `state` key path, then call this method. Alternatively, you can add an
 observer for `DingiOfflinePackProgressChangedNotification` about this pack that
